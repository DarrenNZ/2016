...
public class ImageProcessor{
        ...
        for(int row = 0; row< this.image.length; row++){
            for(int col = 0; col<this.image[0].length; col++){
                if(this.image[row][col]< 128){
                    this.image[row][col]*=.8;
                }
                else{
                    this.image[row][col]*=1.2;
    ...
    public void flipImageHorizontally(){
        int[][] spare = new int [][]{{80,80,80},{80, 200, 80},{80,80,80}};
        spare = this.image;
        for(int row = 0; row< this.image.length; row++){
            for(int col = 0; col<this.image[0].length/2; col++){
                int temp = spare[row][col];
                spare[row][col]= this.image[row][spare[0].length - col - 1];
                spare[row][spare[0].length - col - 1]= temp;
    ...
    public void flipImageVertically(){
        for(int row = 1; row < this.image.length/2; row++){
            //for(int col = 0; col<this.image[0].length;col++){
            int[] temp = this.image[row];
            this.image[row]=this.image[this.image.length -row -1];
            this.image[this.image.length-row-1] = temp;
            //}
        }
    ...
    public void rotateImage180(){
        /**int[][] spare = new int[][]{{80,80,80},{80, 200, 80},{80,80,80}}; for (int row = 0; row< this.image.length; row++){ for(int col = 0; col<this.image[0].length; col++){ spare = this.image; this.image[row][this.image.length - col - 1]=spare[col][row]; } } int[][] matrix = new int[][]{{1,2,3},{4,5,6},{7,8,9}}; //hold //{{,,},{,,},{,,}} int[][] test = new int[0][0]; for (int x = 0; x<matrix.length;x++){ for(int y = 0; y<matrix[0].length;y++){ test[y][matrix[0].length-1-x]=matrix[x][y]; } } int row = 0; int col = 0; UI.println(test[row][col]+" | "+ test[row][col+1]+ " | "+test[row][col+2]); UI.println(test[row+1][col]+" | "+ test[row+1][col+1]+ " | "+test[row+1][col+2]); UI.println(test[row+2][col]+" | "+ test[row+2][col+1]+ " | "+test[row+2][col+2]); this.redisplayImage();**/
        int[][] result = new int [this.image.length][this.image[0].length];

        for (int x = 0; x<this.image.length-1;x++){
            for(int y = 0; y<this.image[0].length;y++){
                result[x][y]= this.image[this.image.length-1-x][y];
    ...
    public void rotateImage90(){
        int[][] result = new int [this.image[0].length][this.image.length];

        for (int x = 0; x<this.image.length;x++){
            for(int y = 0; y<this.image[0].length;y++){
                result[y][this.image.length-1-x]=this.image[x][y];
    ...
    public void expandImage(){
       ...
       int xx = 0;
       int yy = 0;
        for (int x = 0; xx < this.image.length-1;x++){
            yy=0;
            for(int y = 0; yy < this.image[0].length;y++){

                result[x][y] = this.image[x][y];
                result[xx][yy+1] = this.image[x][y];
                result[xx+1][yy] = this.image[x][y];
                result[xx+1][yy+1] = this.image[x][y];
    ...
    public void mergeImage(){
        ...
        for (int x = 0; x < rows-1; x++){
            ...
            holder[x][y] = ((this.image[x][y]+ other[x][y])/2);
